import {
  colorDefinitions,
  colorDefinitionsDark,
  colorDefinitionsLight,
  colorValues,
} from "./pdfStyles";

// Get color by perplexity (matching your UI logic)
// isDark parameter controls whether to use dark mode or light mode colors
export const getColorByPerplexity = (
  highlight_sentence_for_ai,
  perplexity,
  isDark = false,
) => {
  const p = parseInt(perplexity);
  const colors = isDark ? colorDefinitionsDark : colorDefinitionsLight;

  if (highlight_sentence_for_ai) {
    if (p >= colorValues?.aiHigh) return colors.aiHigh;
    if (p >= colorValues?.aiMedium) return colors.aiMedium;
    if (p >= colorValues?.aiLow) return colors.aiLow;
    return colors.aiLow;
  } else {
    if (p <= colorValues?.humanHigh) return colors.humanHigh;
    if (p <= colorValues?.humanMedium) return colors.humanMedium;
    if (p <= colorValues?.humanLow) return colors.humanLow;
    return colors.humanLow;
  }
};
