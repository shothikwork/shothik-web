...
<body>${content}</body>
    ...
    const calibreRes = await fetch(`${CALIBRE_URL}/convert`, {
      ...
      body: JSON.stringify({ format: "docx", html_content: styledHtml, title, author }),
    ...
    if (!calibreRes.ok) {
      ...
      return NextResponse.json(
        { error: (err as any).detail || "DOCX conversion failed" },
        { status: calibreRes.status }
      );
    }

    const data = await calibreRes.json() as { file_base64: string };
    const docxBytes = Buffer.from(data.file_base64, "base64");
    ...
    if (err?.name === "TimeoutError") {
      return NextResponse.json({ error: "DOCX generation timed out" }, { status: 504 });
    }
